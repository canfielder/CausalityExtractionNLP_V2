...
```{r}
  ...
  # Maintain Data 
  hypothesis_causality_input <- h_statements

  # Drop ~Hypo #:~
  hypothesis <- gsub(".*: ","",h_statements)
```
```{r}
...
if ("__label__0" %in% col_names) {
  ...
  hypothesis_causality_input <- hypothesis_causality_input[response]
  hypothesis <- hypothesis[response]
}

hypothesis_causality_input
```


```{r}
# Convert Text
  hypothesis <- str_to_sentence(hypothesis, locale = "en")
```


```{r}
  # Create Dataframe with Hypothesis Number and Hypothesis
  df_hypothesis <- data.frame(hypothesis,
                                 hypothesis_causality_input, 
                              stringsAsFactors = FALSE)
